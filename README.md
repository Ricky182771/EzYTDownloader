...
UI & Core
C++20 / Qt6
Main window, orchestration of asynchronous processes (QProcess), and state management.
Backend
Python 3
Wrapper for yt-dlp responsible for fetching metadata and managing downloads via a JSON-lines protocol.
Conversion
ffmpeg
Remuxing of native formats (e.g., .webm) to final .mp4, .mkv, or .mp3 files.
...
Usage
Launch EzYTDownloader.
Paste the video URL into the input field and click Fetch to retrieve available formats.
Select the desired container (MP4, MKV, or MP3) and the target resolution.
Define the output directory.
Click Download. The application will automatically handle the download and subsequent conversion.
License
